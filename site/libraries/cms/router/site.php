...
class JRouterSite extends JRouter
{
	...
	public function parse($uri)
	{
		...
		//leo
		$reals = $app->getCfg('real_site');
		if( $GLOBALS['debugindex']) {
			JLog::add("path before:$path real=$reals "  ,JLog::DEBUG, "cloudtime" );		
		}
		if (! $reals ) {
			$baseu = JURI::base(true);
			// Remove the base URI path.
			$path = substr_replace($path, '', 0, strlen($baseu));
		} else {
			$path = substr_replace($path, '', 0, strlen($reals));
		}
		if( $GLOBALS['debugindex']) {
			JLog::add("path after:" . $path  ,JLog::DEBUG, "cloudtime" );
		}
