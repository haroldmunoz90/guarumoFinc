...
class JAccess
{
	 ...
	 * @since   11.1
	 */
	public static function getAuthorisedViewLevels($userId,$enterprise_id = null)
	{
		...
		if (empty(self::$viewLevels))
		{
			...
			if( !$enterprise_id ) {
				...
				$query = $db->getQuery(true)
					->select('id, rules')
					->from($db->quoteName('#__viewlevels', "vl"))
					->innerJoin(" ilg_viewlevels_enterprise vle on vle.viewlevels_id = vl.id" )
					->where(" vle.enterprise_id = ".$db->quote($enterprise_id));
			}
