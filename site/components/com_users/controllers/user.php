...
class UsersControllerUser extends UsersController {
    ...
    public function login() {
		if( $_REQUEST["ctcredirect"] != "29jdvksndueru3848jdnvn" ) {
        ...
        if (true === $app->login($credentials, $options)) {
            ...
            $user = BaseComp::getLoggedUser();
			if( BaseCore::getLoggedEnterprise()->status_id != 1 ) {
				$app->logout();
				$app->redirect(JRoute::_('index.php?option=com_users&view=login', false));
			}			
			$articleChangePassword = $GLOBALS['articleChangePassword'];
            if ($user->pass_changed == 0) {				
                $app->redirect(JRoute::_("index.php?Itemid=".$articleChangePassword, false));
            } else {
                $lrt = strtotime($user->lastResetTime);
                $nt = time();
                $datediff = $nt - $lrt;
