...
abstract class YtFrameworkTemplate {
		...
		if (isset($_COOKIE[$this->template.'_tpl']) && $_COOKIE[$this->template.'_tpl'] == $this->template){
			foreach($this->_params_cookie as $k=>$v) {
				...
				if (JRequest::getVar($k)!= null){
					$v = JRequest::getVar($k);
                        @setcookie($kc, $v, $exp, '/', null, $secure, true);
				}else{
					if (isset($_COOKIE[$kc])){
						$v = $_COOKIE[$kc];
					}
                ...
                @setcookie($this->template . '_tpl', $this->template, $exp, '/', null, $secure, true);
		}
		return $this;
	}
	// Get param template
	...
	function isOverrideTemplate(){
		if(JRequest::getVar('tmpl')!='component'){ //var_dump(JSite::getMenu()->getActive()); die();
			$menuid = '';
                if (is_object(JSite::getMenu()->getActive()))
	...
	function ytStyleSheet($url){
		...
		if(($this->getParam('developing', 0)==1 || JRequest::getVar('less2css')=='all') && file_exists($lessurl)){
			...
			$doc->addStyleSheet($url);
                ...
                if (array_key_exists('soran_template_config', $GLOBALS)) {
                    if (array_key_exists('url_to_soran_site', $GLOBALS)) {
                        $doc->addStyleSheet($GLOBALS['url_to_soran_site']."/css/template/".$GLOBALS['soran_template_config']."Template.css");
                    } else {
                    	$doc->addStyleSheet("libraries/soran/css/template/" . $GLOBALS['soran_template_config'] . "Template.css");
                    }
                } else {
	...
	function getLogo(){
		...
		if ($this->getParam('logoType')=='image'):  
                if (array_key_exists('soran_template_config', $GLOBALS)) {
                    if (array_key_exists('url_to_soran_site', $GLOBALS)) {
                        $url = $GLOBALS['url_to_soran_site']."/css/template/images/".$GLOBALS['soran_template_config']."/".$GLOBALS['soran_template_config']."Logo.png";
                    } else {
                    	$url = "libraries/soran/css/template/images/" . $GLOBALS['soran_template_config'] . "/" . $GLOBALS['soran_template_config'] . "Logo.png";
                    }
	...
	function renPositionsGroup($position, $type='', $special = null){
		...
		if ( $position['type'] == 'modules' ) { 
			...
			if ( $has_suffix ) {
                ...
                if ($this->getParam('hideComponentHomePage') == '1' && $this->isHomePage()) {
			?>
