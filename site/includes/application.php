	...
	public function dispatch($component = null)
	{
		...
		switch ($document->getType())
		{
			case 'html':
				...
				{
					$reals = $this->getCfg('real_site');
					if (! $reals ) {
						$document->setBase(htmlspecialchars(JURI::current()));
					} else {
						$current = JURI::current();
						$current = str_replace($reals, JURI::base(true), $current);
						if( $this->getCfg('force_ssl') >= 1 ) {
							$current = str_replace("http:", "https:", $current);
						}
						$document->setBase(htmlspecialchars( $current ));
					}
				}
				break;
		...
		//if( $component === NULL ) {
		//	JLog::add(__FUNCTION__ . " component=$component:"   ,JLog::ERROR, "cloudtime" );
		//	$url = 'index.php';
